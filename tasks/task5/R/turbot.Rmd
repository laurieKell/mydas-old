...
set.seed(1234)
omYr=om
m(omYr)=m(om)%*%rlnoise(nits,iter(m(om)[1,],1)%=%0,sd=0.2,b=0.9,what="year")
omYr=fwd(omYr,fbar=fbar(om)[,-1],sr=eq,residuals=srDev)
save(omYr,file=file.path(dirDat,"omTurbotYr.RData"),compress="xz")

set.seed(1234)
omYc=om
m(omYc)=m(om)%*%rlnoise(nits,iter(m(om)[1,],1)%=%0,sd=0.2,b=0.0,what="year")
omYc=fwd(omYc,fbar=fbar(omYc)[,-1],sr=eq,residuals=srDev)
save(omYc,file=file.path(dirDat,"omTurbotYc.RData"),compress="xz")

plot(FLStocks("Year"=omYr,"Cohort"=omYc,"Recruitment"=om))
```

**Figure `r iFig=iFig+1; iFig`** Comparisons of Time series for different M.
