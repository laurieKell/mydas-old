...
dirMy="/media/laurence/CE78-43CC7/casablanca"
#dirMy="D:/casablanca"
dirInp=file.path(dirMy,"inputs")

#source('d:/mpb/R/biodyn-fit.R')
...
## Simulate CPUE and Indices of abudance 
...
set.seed(12345)
u  =(stock(simBd)[,-50]+stock(simBd)[,-1])/2
u  =rlnorm(1,log(stock(simBd)),0.2)
```
...
## Biomass Dynamic Assessment

```{r}
bd=biodyn(catch=ctc[,-(49:50)],indices=u[,-(49:50)])

params(bd)[c("r","k","b0")]=c(.3,1500,1)
...
plot(bd)
```
**Figure `r iFig=iFig+1; iFig`** Biomass dynamic estimates of stock, harvest rate and catch
...
plotProduction(bd)+
  geom_path( aes(stock,catch),
             model.frame(mcf(FLQuants(bd,"stock","catch"))))+
  geom_point(aes(stock,catch),
             model.frame(mcf(FLQuants(bd,"stock","catch"))))+
  theme_bw()+theme(legend.position="none")
```
**Figure `r iFig=iFig+1; iFig`** Production function with stock trajectory 
...
```{r prfl-r, eval=FALSE}
bdr=profile(bd,which='r',fixed=c('b0','p'),range=seq(0.95,1.03,.002))

ggplot(ddply(bdr,.(r), with, data.frame(ll=sum(ll))))+
  geom_line(aes(r,ll))  +
  theme_bw()
```
...
```{r prfl-k, eval=FALSE}
bdk=profile(bd,which='k',fixed=c('b0','p'),range=seq(0.95,1.03,.002))

ggplot(ddply(bdk,.(k), with, data.frame(ll=sum(ll))))+
  geom_line(aes(k,ll))  +
  theme_bw()
```
...
ret=as(mlply(data.frame(year=45:49),function(year) retro(bd,year)),"biodyns")
